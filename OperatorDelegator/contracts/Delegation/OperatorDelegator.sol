    ...
    OperatorDelegatorStorageV12
{
    ...
    /**
     * @notice  reinitializing the OperatorDelegator to track 0 out the podDelta, reinitialize with version 4
    ...
    function reinitialize(
        ...
        // reset exit balance for the incorrect consolidated amount tracking
    ...
    function requestConsolidation(
        ...
        if (requests.length + consolidatingValidators.length() > 10) {
            revert MaxConsolidationRequested();
        }
        // track and get total consolidation amount
        totalConsolidatedAmount += OperatorDelegatorLib.getTotalConsolidationAmount(
            requests,
            eigenPod,
            consolidatingValidators
        );

        // request consolidation on eigenPod
    ...
    function verifyConsolidationComplete(
        ...
        // verify consolidation complete on eigenPod
        OperatorDelegatorLib.verifyConsolidationComplete(
    ...
    function emergencyTrackMissedCheckpoint(
        ...
        (totalBeaconChainExitBalance, totalConsolidatedAmount) = OperatorDelegatorLib
            .updateTotalBeaconChainExitBalance(
    ...
    function startCheckpoint() external onlyNativeEthRestakeAdmin {
        ...
        // check for any existing consolidating validators
        if (consolidatingValidators.length() > 0) revert PendingConsolidatingValidators();
